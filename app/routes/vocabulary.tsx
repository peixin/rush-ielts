...
export default function Vocabulary() {
  ...
  const handleAdd = () => {
    setEditingWord({ word: "", definition: [], examples: [], phonetic: "" });
    setIsEditModalOpen(true);
  };
        ...
        <div className="max-w-3xl mx-auto flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Link to="/" className="text-gray-500 hover:text-gray-900 dark:hover:text-white font-bold text-xl">
                 ←
              </Link>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Vocabulary</h1>
            </div>
          ...
          <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 -mb-3.5">
            ...
            {currentProgressId !== undefined && (
                <button 
                    onClick={() => {
                        if(confirm("Reset progress? This will clear your bookmark.")) {
                            const s = getUserSettings();
                            if(s) saveUserSettings({ ...s, lastStudiedWordId: undefined });
                            setCurrentProgressId(undefined);
                        }
                    }}
                    className="text-xs text-red-500 hover:text-red-700 font-semibold pb-3 mb-1"
                >
                    Reset Progress
                </button>
            )}
