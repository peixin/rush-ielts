...
export default function Study() {
  ...
  const mode = (searchParams.get("mode") as Mode) || "recognition";
  ...
  useEffect(() => {
    async function load() {
      ...
      if (source === "mistakes") {
        ...
        // Filter words after the last studied one
        words = allWords.filter(w => (w.id || 0) > lastId);
        
        // For standard study, we keep sequential order to track progress
        setQueue(words);
      }
    ...
    return (
      <div className="flex-grow flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-gray-500 mb-8">
          {source === "mistakes" ? "Great job! You have no pending mistakes." : "You have reviewed all available vocabulary up to this point."}
        </p>
        <div className="flex gap-4">
            <Link to="/" className="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-900 rounded-lg font-semibold transition-colors">
            Back Home
            </Link>
            {source === "all" && (
                <button 
                    onClick={() => {
                        ...
                        if(s) saveUserSettings({ ...s, lastStudiedWordId: 0 });
  ...
  if (isComplete) {
    return (
      <div className="flex-grow flex flex-col items-center justify-center p-6 text-center bg-green-50 dark:bg-green-900/20">
        ...
        <Link to="/" className="px-8 py-4 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold shadow-lg transition-transform active:scale-95">
          Back Home
        </Link>
      </div>
    );
  ...
  return (
    <div className="flex-grow bg-gray-100 dark:bg-gray-950 flex flex-col font-sans">
      ...
      <div className="p-4 flex justify-between items-center text-sm text-gray-500">
        <Link to="/" className="hover:text-gray-900 dark:hover:text-gray-300">✕ Exit</Link>
